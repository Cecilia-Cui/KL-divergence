...
def run(M1,M2,Ra,Dec,Dist,iota,Pol,dt,F_min,Det,delta_Ra,delta_Dec,Re):
    ...
    FM = np.zeros((2, 2))  # Modified to 2x2 for M1 and M2
    SNR = np.zeros(len(Det))
    SNRw = np.zeros([len(Det), pieces])
    wholeFM = np.array([np.array(np.zeros((2,2))) for i in range(pieces)])
    ...
    for piece in range(pieces):
        FM_sub = np.zeros((2, 2))  # 2x2 sub-matrix for M1 and M2
        if (piece + 1) % 100 == 0:
        ...
        for detector in range(NW):
            ...
            for h in range(2):
                for j in range(2):
                    FM_sub[0, 0] += 2.0 * df * (np.vdot(dh_deri[0], dh_deri[0])
                           + np.vdot(dh_deri[0], dh_deri[0]))
                    FM_sub[0, 1] += 2.0 * df * (np.vdot(dh_deri[0], dh_deri[1])
                           + np.vdot(dh_deri[1], dh_deri[0]))
                    FM_sub[1, 0] += 2.0 * df * (np.vdot(dh_deri[1], dh_deri[0])
                           + np.vdot(dh_deri[0], dh_deri[1]))
                    FM_sub[1, 1] += 2.0 * df * (np.vdot(dh_deri[1], dh_deri[1])
                           + np.vdot(dh_deri[1], dh_deri[1]))
        ...
        FM += FM_sub  # Accumulate 2x2 sub-matrix
